...
cursor=conn.cursor()
cursor.execute("select count(*) from sys.databases where name='LidarData2'")
if(int(cursor.fetchone()[0])>0):
    cursor.commit()
    cursor.execute("drop database LidarData2")
cursor.commit()
cursor.execute('create database LidarData2')
cursor.commit()
cursor.execute('use LidarData2')
cursor.commit()
